...
2. Si quieres ver la especificación, abre <http://localhost:9917/docs> o descarga <http://localhost:9917/openapi.yaml>.

3. En otra terminal, abre SSE:

```bash
curl -N "http://localhost:9917/v1/stream?since=0&types=state_updated,state_deleted,vector_added,vector_upserted,vector_updated,vector_deleted,gap" ^
```
